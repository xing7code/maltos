"""Roundtrip test for RuntimeCore ZeRO-3 sharded checkpointing.

The test trains one local step, saves rank-local ZeRO3 shards, restores them
into a fresh RuntimeCore with the same mesh/plugin setup, then materializes both
runtimes and compares logical parameters.
"""
...
def _build_core(model: TinyModel, world_size: int) -> tuple[RuntimeCore, Zero3Plugin]:
    zero3 = Zero3Plugin(enable_prefetch=True, optimizer_cls=torch.optim.SGD, lr=_LR)
    core = RuntimeCore(
...
def _run_worker(rank: int, args: argparse.Namespace) -> None:
    ...
    full_batch = torch.randn(args.global_batch_size, args.hidden_size)
    ...
    save_sharded_checkpoint(trained_core, args.checkpoint_dir)
    ...
    trained_zero3.materialize_model()
    restored_zero3.materialize_model()
    param_name, param_diff = _max_param_diff(trained_core.model, restored_core.model)

    if rank == 0:
        print(f"Checkpoint dir    : {args.checkpoint_dir}")
        print(f"Roundtrip diff    : {param_diff:.2e}  ({param_name}, atol={_ATOL:.2e})")
        if param_diff > _ATOL:
            raise AssertionError(f"ZeRO3 checkpoint roundtrip failed: param={param_name}, diff={param_diff:.2e}")
        print("PASS")

    trained_zero3.reshard_model()
...
def main() -> None:
    ...
    if args.checkpoint_dir is None:
        args.checkpoint_dir = tempfile.mkdtemp(prefix="zero3_ckpt_")
