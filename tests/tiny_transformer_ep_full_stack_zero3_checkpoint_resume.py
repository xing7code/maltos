"""Resume test for full-stack PP+CP+TP+SP+EP+ZeRO3 RuntimeCore checkpointing."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--master-port", type=int, default=29645)
...
def _build_reference(seed: int, batch_size: int, seq_len: int) -> tuple[TinyMoETransformer, torch.Tensor, torch.Tensor]:
    ...
    for module_name, module in model.named_modules():
        ...
        for local_idx, global_idx in enumerate(module.local_expert_ids):
            ...
            for param_name, param in expert.named_parameters():
                ...
                tensors[full_name] = param.detach().clone().cpu()
    return tensors
...
def _logical_named_tensors(
    ...
    for name, param in model.named_parameters():
        if ".local_experts." in name:
            continue
        shared_tensors[name] = _logical_tensor(name, param.detach(), shard_rules, tp_group).cpu()
...
def _build_runtime(model: TinyMoETransformerTpSp, args: argparse.Namespace, device: torch.device | None = None) -> tuple[RuntimeCore, Zero3Plugin]:
...
def _run_worker(rank: int, args: argparse.Namespace) -> None:
    ...
    reference_model, first_tokens, second_tokens = _build_reference(args.seed, args.global_batch_size, args.seq_len)

    continuous_model = TinyMoETransformerTpSp(**_MODEL_KWARGS)
    ...
    dp_idx, _, _, _ = continuous_core.mesh.rank_coordinates(rank)
    local_batch_size = args.global_batch_size // args.dp_size
    first_local = first_tokens.narrow(0, dp_idx * local_batch_size, local_batch_size).contiguous()
    second_local = second_tokens.narrow(0, dp_idx * local_batch_size, local_batch_size).contiguous()

    _, should_step = continuous_core.run_step(causal_lm_batch(first_local))
    if not should_step:
        raise AssertionError("continuous full-stack EP resume expects grad_accum_steps=1")
    ...
    if not should_step:
        raise AssertionError("continuous second EP full-stack step should step optimizer")
    continuous_core.step_optimizer()
    dist.barrier()

    restored_model = TinyMoETransformerTpSp(**_MODEL_KWARGS)
    ...
    if not should_step:
        raise AssertionError("restored second EP full-stack step should step optimizer")
    restored_core.step_optimizer()
    dist.barrier()

    continuous_loss = _reduce_loss(continuous_second_loss, continuous_core)
    restored_loss = _reduce_loss(restored_second_loss, restored_core)
    ...
    pp_group = continuous_core.get_group(MeshAxis.PP)
    ep_group = continuous_core.get_group(MeshAxis.EP)
    continuous_params = _logical_named_tensors(continuous_core.model, shard_rules, tp_group, pp_group, ep_group)
    restored_params = _logical_named_tensors(restored_core.model, shard_rules, tp_group, pp_group, ep_group)
    ...
    if rank == 0:
        ...
        if loss_diff > _LOSS_ATOL:
            raise AssertionError(f"loss mismatch after EP full-stack resume: diff={loss_diff:.2e}")
        if param_diff > _STEP_ATOL:
            raise AssertionError(f"param mismatch after EP full-stack resume: {param_name} diff={param_diff:.2e}")
...
def main() -> None:
    ...
    if args.checkpoint_dir is None:
        args.checkpoint_dir = tempfile.mkdtemp(prefix="ep_full_stack_zero3_resume_")
