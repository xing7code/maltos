"""Equivalence test: full-batch TinyModel baseline vs RuntimeCore ZeRO-2 local grad clip.

Verifies that ZeRO-2 computes a global gradient norm across all ranks and clips
uniformly while gradients remain sharded.

Usage:
  PYTHONPATH=. .venv/bin/python tests/tiny_model_zero2_grad_clip_equivalence.py
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--master-port", type=int, default=29548)
...
def _run_worker(rank: int, args: argparse.Namespace) -> None:
    ...
    core = RuntimeCore(
        ...
        plugins=[Zero2Plugin(bucket_mb_size=0)],
    )
    core.setup()
    ...
    if rank == 0:
        ...
        print(f"RuntimeCore ZeRO2+clip  : {avg_loss.item():.6f}")
        ...
        if loss_diff > _ATOL:
            raise AssertionError(f"ZeRO2+clip loss equivalence failed: diff={loss_diff:.2e}")
        if param_diff > _ATOL:
            raise AssertionError(
                f"ZeRO2+clip one-step equivalence failed: param={param_name}, diff={param_diff:.2e}"
            )
        print("PASS")
