...
Usage:
  PYTHONPATH=. .venv/bin/python tests/tiny_model_zero1_runtime_core_equivalence.py \
    --world-size 2
"""
...
def _run_worker(rank: int, args: argparse.Namespace) -> None:
    ...
    core = RuntimeCore(
        ...
        plugins=[Zero1Plugin(bucket_mb_size=args.bucket_mb_size)],
    )
    core.setup()
    ...
    if rank == 0:
        ...
        print(f"RuntimeCore ZeRO1 : {avg_loss.item():.6f}")
        print(f"Loss diff         : {loss_diff:.2e}  (atol={_ATOL:.2e})")
        print(f"Post-step diff    : {param_diff:.2e}  ({param_name}, atol={_ATOL:.2e})")
        if loss_diff > _ATOL:
            raise AssertionError(f"ZeRO1 loss equivalence failed: diff={loss_diff:.2e}")
        if param_diff > _ATOL:
            raise AssertionError(f"ZeRO1 one-step equivalence failed: param={param_name}, diff={param_diff:.2e}")
        print("PASS")
