"""Equivalence test: full-batch TinyModel vs RuntimeCore ZeRO.

Each rank sees a different local batch. The selected ZeRO plugin must match a
single-process full-batch baseline after one optimizer step.

Usage:
  PYTHONPATH=. .venv/bin/python tests/tiny_model_zero_runtime_core_equivalence.py \
    --zero-stage 1 \
    --world-size 2
"""
...
def _make_zero_plugin(args: argparse.Namespace) -> Zero1Plugin | Zero2Plugin | Zero3Plugin:
    if args.zero_stage == 1:
        return Zero1Plugin(bucket_mb_size=args.bucket_mb_size)
    if args.zero_stage == 2:
        return Zero2Plugin(bucket_mb_size=args.bucket_mb_size)
    return Zero3Plugin()
...
def _run_worker(rank: int, args: argparse.Namespace) -> None:
    ...
    if rank == 0:
        ...
        print(f"RuntimeCore ZeRO{args.zero_stage} : {avg_loss.item():.6f}")
        print(f"Loss diff         : {loss_diff:.2e}  (atol={_ATOL:.2e})")
        print(f"Post-step diff    : {param_diff:.2e}  ({param_name}, atol={_ATOL:.2e})")
        if loss_diff > _ATOL:
            raise AssertionError(f"ZeRO{args.zero_stage} loss equivalence failed: diff={loss_diff:.2e}")
        if param_diff > _ATOL:
            raise AssertionError(f"ZeRO{args.zero_stage} one-step equivalence failed: param={param_name}, diff={param_diff:.2e}")
        print("PASS")
