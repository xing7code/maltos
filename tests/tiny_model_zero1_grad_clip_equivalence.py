"""Equivalence test: full-batch TinyModel baseline vs RuntimeCore ZeRO-1 local grad clip.

Verifies that ZeRO-1 computes a global gradient norm across all ranks and clips
uniformly while gradients remain sharded.

Clip threshold is set to 50 % of the baseline gradient norm so that rank 0's
local shard norm (≈ 45 % of global) falls below the threshold.  Before the fix
rank 0 would not clip at all while rank 1 and the baseline both clip to 0.5 ×
global — producing post-step parameter differences on the order of 1e-4.  After
the fix all ranks share the global norm and produce identical updates.

Usage:
  PYTHONPATH=. .venv/bin/python tests/tiny_model_zero1_grad_clip_equivalence.py
"""
...
def _run_worker(rank: int, args: argparse.Namespace) -> None:
    ...
    if rank == 0:
        ...
        print(f"RuntimeCore ZeRO1+clip  : {avg_loss.item():.6f}")
        ...
        if loss_diff > _ATOL:
            raise AssertionError(f"ZeRO1+clip loss equivalence failed: diff={loss_diff:.2e}")
        if param_diff > _ATOL:
            raise AssertionError(
                f"ZeRO1+clip one-step equivalence failed: param={param_name}, diff={param_diff:.2e}"
            )
        print("PASS")
