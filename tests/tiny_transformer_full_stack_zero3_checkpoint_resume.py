"""Resume test for full-stack PP+CP+TP+SP+ZeRO3 RuntimeCore checkpointing."""
...
def _run_worker(rank: int, args: argparse.Namespace) -> None:
    ...
    if not should_step:
        raise AssertionError("continuous full-stack resume expects grad_accum_steps=1")
    ...
    if not should_step:
        raise AssertionError("continuous second full-stack step should step optimizer")
    continuous_core.step_optimizer()
    dist.barrier()
    ...
    if not should_step:
        raise AssertionError("restored second full-stack step should step optimizer")
    ...
    if rank == 0:
        ...
        if loss_diff > _LOSS_ATOL:
            raise AssertionError(f"loss mismatch after full-stack resume: diff={loss_diff:.2e}")
        if param_diff > _STEP_ATOL:
            raise AssertionError(f"param mismatch after full-stack resume: {param_name} diff={param_diff:.2e}")
...
def main() -> None:
    ...
    if args.checkpoint_dir is None:
        args.checkpoint_dir = tempfile.mkdtemp(prefix="full_stack_zero3_resume_")
