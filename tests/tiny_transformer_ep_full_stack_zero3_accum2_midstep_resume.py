"""Mid-step resume test for full-stack PP+CP+TP+SP+EP+ZeRO3 with grad accumulation."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--master-port", type=int, default=29646)
...
def _run_worker(rank: int, args: argparse.Namespace) -> None:
    ...
    if rank == 0:
        ...
        if worst_loss_diff > _LOSS_ATOL:
            raise AssertionError(f"mid-step EP full-stack resume loss mismatch: tag={worst_loss_tag}, diff={worst_loss_diff:.2e}")
        if param_diff > _STEP_ATOL:
            raise AssertionError(f"mid-step EP full-stack resume param mismatch: {param_name} diff={param_diff:.2e}")
...
def main() -> None:
    ...
    if args.checkpoint_dir is None:
        args.checkpoint_dir = tempfile.mkdtemp(prefix="ep_full_stack_zero3_accum2_midstep_resume_")
